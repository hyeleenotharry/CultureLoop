...
public class logService {
    ...
    public List<String> analyzeImagesConcurrently(MultipartFile[] images) throws InterruptedException, ExecutionException {
        try {
            ExecutorService executor = Executors.newFixedThreadPool(Math.min(images.length, 4));  // 동시 4개 처리
            List<Future<List<String>>> futures = new ArrayList<>();

            for (MultipartFile image : images) {
                futures.add(executor.submit(() -> analyzeImageLabels(image)));
            }

            List<String> allLabels = new ArrayList<>();
            for (Future<List<String>> future : futures) {
                allLabels.addAll(future.get());  // 각 Future 결과 취합
            }
    ...
    public List<String> analyzeImageLabels(MultipartFile image) throws IOException {
